...
module Slack
  class ReportService < AlertService
    ...
    def new_query(ref, start_date)
      "COUNT(DISTINCT (case WHEN (created_at > '#{start_date}' AND content->'email' NOT IN (SELECT DISTINCT(content->'email') FROM events WHERE created_at < '#{start_date}' and event_type='sandbox_signup' and content->'email' <> null AND #{like_query(ref)}) AND #{like_query(ref)}) then content->'email' end)) as \"#{ref_builder(ref)[:new]}\","
    end
    ...
    def build_query(span, start_date)
      "SELECT #{generate_query(start_date).join(' ')} COUNT(DISTINCT(case WHEN (created_at > '#{start_date}' AND content->'email' NOT IN (SELECT DISTINCT(content->'email') FROM events WHERE created_at < '#{start_date}' and event_type='sandbox_signup' and content->'email' <> null)) then content->'email' end)) as #{span}ly_count, COUNT(DISTINCT content->'email') as total_count FROM events WHERE event_type='sandbox_signup';"
    end
