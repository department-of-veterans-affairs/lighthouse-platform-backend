...
class ElasticsearchService
  ...
  def first_successful_call(consumer)
    req = Net::HTTP::Get.new(@uri)
    kong_id = consumer.consumer_auth_refs.kept.find_by(key: ConsumerAuthRef::KEYS[:sandbox_gateway_ref])&.value
    cid = consumer.consumer_auth_refs.kept.find_by(key: ConsumerAuthRef::KEYS[:sandbox_acg_oauth_ref])&.value
