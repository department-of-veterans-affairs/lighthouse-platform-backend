...
describe GithubAlertsController, type: :request do
  let(:valid_params) do
    {
      alert: {
  ...
  it 'responds with a 204 when passed a valid body' do
    VCR.use_cassette('github_alert', match_requests_on: [:method]) do
      post('/platform-backend/github_alerts', params: valid_params)
      expect(response.code).to eq('204')
    end
  end

  it 'responds with an exception when passed an invalid body' do
    post('/platform-backend/github_alerts', params: { boop: 'bap' })
