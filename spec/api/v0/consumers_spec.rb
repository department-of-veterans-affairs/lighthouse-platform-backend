...
describe V0::Consumers, type: :request do
  ...
  let(:sandbox_env_id) { Environment.find_by(name: 'sandbox')[:id] }
  let(:production_env_id) { Environment.find_by(name: 'production')[:id] }
  ...
  describe 'lists consumer APIs' do
    let(:user) { create(:user) }
    ...
    context 'with production environments' do
      ...
      it 'returns a list of production APIs' do
        get "/platform-backend/v0/consumers/#{production_consumer[:id]}/environments/#{production_env_id}/apis"
